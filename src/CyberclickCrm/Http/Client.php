...
use CyberclickCrm\Http\Adapter\AdapterInterface;
use CyberclickCrm\Http\Adapter\CurlAdapter;
use CyberclickCrm\Http\Exception\EmptyResponseException;
use CyberclickCrm\Http\Exception\RequestException;
...
class Client {
   ...
   * @var string
   */
  const DEFAULT_GRAPH_BASE_DOMAIN = 'cyberclick.es';

  /**
   * @var string
   */
  const DEFAULT_LAST_LEVEL_DOMAIN = 'crm';
  ...
  public function getDefaultRequestHeaderds() {
    if ($this->defaultRequestHeaders === null) {
      $this->defaultRequestHeaders = new Headers(array(
        'User-Agent' => 'cyberclick-crm-client-php-v'.Api::VERSION,
        'Accept-Encoding' => '*',
      ));
    }
